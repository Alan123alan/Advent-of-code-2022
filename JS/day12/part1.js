...
const confirmAdjancency = (currentNode, compareNode, maze) => {
...
const sample = fs.readFileSync('./js/day12/input.txt', { encoding: 'utf-8' }).split('\n').map(row => row.split("")).map((row, i) => row.map((cell, j) => {
    if (cell == START) {
        start = { row: i, col: j };
...
const nodePoints = Object.values(nodesMap);
...
console.log(path);
console.log(path === null || path === void 0 ? void 0 : path.length);
